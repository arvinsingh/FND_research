...
class SentimentFeatureGenerator(FeatureGenerator):
    ...
    def process(self, df):
        ...
        df['headline_sents'] = df['Headline'].astype(str).apply(lambda x: sent_tokenize(x))
        ...
        with open("../saved_data/kaggle/" + outfilename_hsenti, "wb") as outfile:
        ...
        print ('for body')
        df['body_sents'] = df['articleBody'].astype(str).map(lambda x: sent_tokenize(x))
        ...
        with open("../saved_data/kaggle/" + outfilename_bsenti, "wb") as outfile:
    ...
    def read(self):
        ...
        with open("../saved_data/new/" + filename_hsenti, "rb") as infile:
            headlineSenti = pickle.load(infile)

        filename_bsenti = "body.senti.pkl"
        with open("../saved_data/new/" + filename_bsenti, "rb") as infile:
