...
class Word2VecFeatureGenerator(FeatureGenerator):
    ...
    def process(self, df):
        ...
        df["Headline_unigram_vec"] = df["Headline"].astype(str).map(lambda x: preprocess_data(x, exclude_stopword=False, stem=False))
        df["articleBody_unigram_vec"] = df["articleBody"].astype(str).map(lambda x: preprocess_data(x, exclude_stopword=False, stem=False))
        ...
        with open("../saved_data/kaggle/" + outfilename_hvec, "wb") as outfile:
        ...
        with open("../saved_data/kaggle/" + outfilename_bvec, "wb") as outfile:
        ...
        with open("../saved_data/kaggle/" + outfilename_simvec, "wb") as outfile:
    ...
    def read(self):
        ...
        with open("../saved_data/new/" + filename_hvec, "rb") as infile:
            headlineVec = pickle.load(infile)

        filename_bvec = "body.word2vec.pkl"
        with open("../saved_data/new/" + filename_bvec, "rb") as infile:
            bodyVec = pickle.load(infile)

        filename_simvec = "sim.word2vec.pkl"
        with open("../saved_data/new/" + filename_simvec, "rb") as infile:
